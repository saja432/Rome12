...
local BallTargetMemory = setmetatable({}, {__mode = "k"})
...
local function allowParryRequest(now)
    ...
    if #State.RequestTimes >= 3 then
        State.GuardUntil = now + 0.65
        return false
    end
...
local function doParry(force)
    local now = os.clock()
    local minGap = force and 0.18 or 0.22
    if now - State.LastParry < minGap then return false end
    if not allowParryRequest(now) then return false end
    -- Respect the game's local Block cooldown. Repeated calls while the button is
    -- unavailable can create a request pattern that the server rejects.
    if not blockReady() then return false end
...
local function parryTick()
    ...
    -- One-shot lock per ball flight. A single ball object can remain in
    -- workspace after a successful parry, so distance/TTI alone must not
    -- cause another remote call every frame.
    local currentTarget = safe(function() return ball:GetAttribute("target") end)
    local previousTarget = BallTargetMemory[ball]
    if previousTarget ~= nil and currentTarget ~= previousTarget then
        BallCooldown[ball] = 0
    end
    BallTargetMemory[ball] = currentTarget

    local lockUntil = BallCooldown[ball] or 0
    if now < lockUntil then return end

    local gap = State.CloseFight and 0.28 or 0.32
    if curveRisk(info) then gap = 0.36 end
...
connect(LocalPlayer.CharacterAdded, function()
    ...
    BallTargetMemory = setmetatable({}, {__mode = "k"})
    State.LastBall = nil
