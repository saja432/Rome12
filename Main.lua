...
local function getRemotes()
    ...
    local parry = remotes:FindFirstChild("ParryButtonPress", true)
...
local function isTargeted(ball, char)
    ...
    if target ~= nil then
        if target == LocalPlayer.Name or target == LocalPlayer.DisplayName or target == tostring(LocalPlayer.UserId) then return true end
        if typeof(target) == "Instance" then
            if target == LocalPlayer or target == char then return true end
            if target:IsA("Player") and target == LocalPlayer then return true end
        end
    end
    for _, key in ipairs({"Target","target","TargetPlayer","targetPlayer","TargetedPlayer"}) do
        local v = safe(function() return ball:GetAttribute(key) end)
        if v == LocalPlayer.Name or v == LocalPlayer.DisplayName or v == tostring(LocalPlayer.UserId) then return true end
    end
...
local function getBestBall(root, char)
    ...
    for _, ball in ipairs(folder:GetChildren()) do
        if ball:IsA("BasePart") and (isRealBall(ball) or ball:GetAttribute("realBall") == nil) then
            local vel = getBallVelocity(ball)
            local offset = root.Position - ball.Position
            local dist = offset.Magnitude
            if dist > 0.001 and vel.Magnitude > 1 then
                ...
                if toward > 0 then
                    ...
                    local score = tti - (targeted and 0.35 or 0)
                    if targeted and score < bestScore then
                        bestScore = score
                        best = ball
                    end
                    if tti < fallbackScore then
                        fallbackScore = tti
                        fallback = ball
                    end
                end
...
local function fireParry()
    ...
    local ok = false
    if parry:IsA("RemoteEvent") then
        ok = pcall(function() parry:FireServer() end)
    elseif parry:IsA("BindableEvent") then
        ok = pcall(function() parry:Fire() end)
    else
        ok = pcall(function()
            if parry.FireServer then parry:FireServer() else parry:Fire() end
        end)
    end
    if ok then State.LastParry = os.clock() end
    return ok
end
...
local function makeToggle(text, y, key)
    ...
    b.MouseButton1Click:Connect(function()
        ...
        if key == "AntiLag" then applyAntiLag(State[key]) end
        render()
    end)
    render()
    return b
